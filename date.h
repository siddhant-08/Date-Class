...
class DateFormat {
	...
	const char* getMF();
	const char* getYF();
	void setDF(const char* x);
	void setMF(const char* x);
	void setYF(const char* x);
	/*-----------------------------------------------------------------------------------------------------*/
...
class Date {
	...
	// CONSTRUCTORS
		...
		Date& operator++()throw(out_of_range); // Next day
		Date& operator++(int)throw(out_of_range); // Same day next week
		Date& operator--()throw(out_of_range); // Previous day
		Date& operator--(int)throw(out_of_range); // Same day previous week

		// BINARY ARITHMETIC OPERATORS
		unsigned int operator-(const Date& otherDate); // Number of days between otherDate and current date
		long long int getnumdays(const Date&);
		Date  days_to_date(long long);
